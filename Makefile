...
build: ## Build the executable. Example: make build
	env GOOS=$(TARGET_GOOS) GOARCH=$(TARGET_GOARCH) CGO_ENABLED=1 go build $(GOFLAGS) -o _output/csi-driver-shared-resource ./cmd/csidriver
.PHONY: build

build-webhook: ## Build the executable. Example: make build
	env GOOS=$(TARGET_GOOS) GOARCH=$(TARGET_GOARCH) CGO_ENABLED=1 go build $(GOFLAGS) -o _output/csi-driver-shared-resource-webhook ./cmd/webhook
.PHONY: build-webhook
