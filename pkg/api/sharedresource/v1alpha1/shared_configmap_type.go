...
// available cluster scoped SharedConfigMaps objects.
//
...
type SharedConfigMap struct {
	...
	// Specification of the desired shared configmap
	// +kubebuilder:validation:Required
	Spec SharedConfigMapSpec `json:"spec,omitempty" protobuf:"bytes,2,opt,name=spec"`
...
type SharedConfigMapList struct {
	...
	Items           []SharedConfigMap `json:"items" protobuf:"bytes,2,rep,name=items"`
}

// SharedConfigMapSpec defines the desired state of a SharedConfigMap
// +k8s:openapi-gen=true
type SharedConfigMapSpec struct {
	// configMap references the backing ConfigMap for this SharedConfigMap.
	// +kubebuilder:validation:Required
	ConfigMap ResourceReference `json:"configmap" protobuf:"bytes,1,opt,name=configmap"`
